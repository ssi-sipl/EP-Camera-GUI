# ===== GUI - Triple Sensor Payload (single page, updated layout) =====
...
class TriplePayloadGUI:
    def __init__(self, root):
        ...
        self.day_process = None
        self.day_streaming = False
        # whether the colour (RGB) day pipeline is active (toggle state)
        ...
        if PIL_AVAILABLE:
            ...
            try:
                ...
                # In case PIL image creation fails for any reason, skip
                pass
    ...
    def _setup_day_pipeline(self, pipeline_str):
        """
        Create a new GStreamer pipeline from pipeline_str and connect appsink name=sink
        pipeline_str must include an appsink named sink (we provide appsink in caller)
        """
        # stop existing pipeline if present
        try:
        ...
        if not self.day_sink:
            # appsink missing or not named; try to find sink by interface
            try:
                self.day_sink = self.day_pipeline.get_by_interface("GstApp.AppSink")
            except Exception:
                self.day_sink = None

        if self.day_sink:
            # configure appsink for callbacks
            ...
            raise RuntimeError("Failed to get appsink 'sink' from pipeline.")

    def _on_day_sample(self, sink):
        print(f"[{time.time():.2f}] _on_day_sample CALLED!")
        ...
        try:
            ...
            # fallback if caps missing
            w, h = 640, 480

        # Use extract_dup which returns bytes for entire buffer
        try:
            data = buf.extract_dup(0, buf.get_size())
        except Exception:
            return Gst.FlowReturn.OK

        # Try to interpret data: it may be RGB (3 channels) or GRAY (1 channel)
        try:
            arr = np.frombuffer(data, np.uint8)
            # If buffer size matches h*w -> grayscale
            if arr.size == (h * w):
                arr = arr.reshape((h, w))
                # convert to RGB for consistent display and overlay steps
        ...
        arr_bgr = cv2.resize(arr_bgr, (widget_w, widget_h), interpolation=cv2.INTER_AREA)

        # Apply crosshair overlay only if enabled, AFTER fixed resize
        if self.crosshair_enabled:
            try:
                arr_bgr = overlay_crosshair(arr_bgr)
        ...
        # Only recreate PhotoImage if shape changed
        try:
            if not hasattr(self, "day_imgtk") or self.day_imgtk.width() != widget_w or self.day_imgtk.height() != widget_h:
                self.day_imgtk = ImageTk.PhotoImage(img)
                target_label.config(image=self.day_imgtk)
            else:
                try:
                    self.day_imgtk.paste(img)  # paste new data into same PhotoImage
                except Exception:
                    self.day_imgtk = ImageTk.PhotoImage(img)
                    target_label.config(image=self.day_imgtk)
        except Exception:
    ...
    def _build_layout(self):
        ...
        # Denoise
        ttk.Label(disp, text="Denoise (0â€“15)").grid(row=2, column=0, sticky="w", pady=(6,0))
    ...
    def thermal_read_info(self):
        self._set_status("Reading thermal camera infoâ€¦")
    ...
    def day_start_stream(self):
        """Start black & white day pipeline (requested: 960x720@10) as appsink so GUI can overlay crosshair."""
        ...
        if self.day_colour_running:
            try:
                ...
                pass
            self.day_colour_running = False
            # update colour button text
            try:
                self.colour_btn_text.set("Start Colour Stream")
            except Exception:
                pass

        # Use appsink so we get frames into Python and can overlay
        # Use GRAY8 to mimic black & white stream; convert to RGB in _on_day_sample
        pipeline_str = (
            "aravissrc ! "
        ...
        try:
            self._setup_day_pipeline(pipeline_str)
            self.day_pipeline.set_state(Gst.State.PLAYING)
            self.day_streaming = True
            self._set_status("Day camera (B/W) streaming started.")
        except Exception as e:
            self._set_status(f"Day start error: {e}")

    def day_start_color_stream(self):
        """Deprecated - kept for compatibility; use toggle_day_colour_stream instead."""
        return self.toggle_day_colour_stream()

    def toggle_day_colour_stream(self):
        """Toggle the colour (RGB) day pipeline on/off when user presses the Colour Stream button."""
        ...
        try:
            ...
            self._set_status(f"Day colour start error: {e}")
    
    # --- Day Stop Stream Fix (Updated) ---
    def day_stop_stream(self):
        """Stop day camera stream completely - non-blocking version"""
        if not self.day_streaming and not self.day_colour_running:
            self._set_status("Day camera stream not running.")
            return

        # Step 1: Set flags immediately to stop frame processing
        self.day_streaming = False
        self.day_colour_running = False
        
        # Step 2: Update GUI immediately (optimistic update)
        try:
            if hasattr(self, 'day_black_img') and self.day_black_img:
                self.day_video_label.config(image=self.day_black_img)
                self.day_video_label.imgtk = self.day_black_img
            else:
                self.day_video_label.config(image="", text="Camera Stopped", fg="white", bg="black")
            
            if hasattr(self, 'colour_btn_text'):
                self.colour_btn_text.set("Start Colour Stream")
                
            self._set_status("Day camera stopping...")
        except Exception as e:
            self._set_status(f"GUI update error: {e}")

        # Step 3: Stop pipeline safely - SYNCHRONOUS approach for restart reliability
        if self.day_pipeline:
            try:
                # Disconnect signal handler first
                if self.day_sink:
                    try:
                        self.day_sink.disconnect_by_func(self._on_day_sample)
                    except:
                        pass
                
                # Force immediate NULL state for reliable restart
                self.day_pipeline.set_state(Gst.State.NULL)
                
                # Wait for NULL state with short timeout
                state_ret, state, pending = self.day_pipeline.get_state(3 * Gst.SECOND)
                
                if state_ret == Gst.StateChangeReturn.TIMEOUT:
                    print("Warning: Pipeline stop timeout - pipeline may not restart properly")
                
                # Clean up references immediately
                self.day_pipeline = None
                self.day_sink = None
                if hasattr(self, 'day_imgtk'):
                    self.day_imgtk = None
        ...
        self._set_status("Day camera stopped.")

    def _cleanup_pipeline_references(self):
        """Clean up pipeline references (called on main thread)"""
        try:
            self.day_pipeline = None
            self.day_sink = None
            if hasattr(self, 'day_imgtk'):
    ...
    def _day_monitor_process(self):
        if not self.day_process:
            return
        try:
            for line in iter(self.day_process.stderr.readline, b''):
                if not self.day_streaming:
                    break
                decoded = line.decode('utf-8', errors='ignore').strip()
                if decoded:
                    self.root.after(0, lambda m=decoded: self._set_status(f"Day/GST: {m}"))
        except Exception:
            pass
